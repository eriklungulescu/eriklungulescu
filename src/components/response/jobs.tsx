...
export const Jobs = () => {
    return <>
        <JobEntry 
            ...
            stack={[{name: 'Java', color: '#e78b2e'}, {name: 'Spring Boot', color: '#33b727'}, {name: 'Node.js', color: '#308728'}, {name: 'Express.js', color: '#2b2d2b'}, {name: 'Jaeger', color: '#2fb8c2'}, {name: 'Docker', color: '#0db7ed'}]}
        ...
        <JobEntry 
            ...
            stack={[{name: 'Java', color: '#e78b2e'}, {name: 'Amazon Web Services', color: '#101010'}, {name: 'Lambda', color: '#ab3890'}, {name: 'S3', color: '#ab3890'}, {name: 'SQS', color: '#ab3890'}, {name: 'Docker', color: '#0db7ed'}]}
            description={`I honestly really enjoyed working for Capital One, which was surprising to me considering it's a bank. I was given the task of creating a tool to archive user credit data in real time. To do this I created a "primary" Lambda which would intercept our DynamoDB insert events, process the data, and then forward it to our Snowflake database. I additionally created a "retry" system to ensure data retention and reliability in case of outages or data processing errors. This system involved the "primary" Lambda sending failed data to an SQS queue, which a "retry" lambda would pickup asyncrhonously and then attempt to archive it again. In the case that even this failed I implemented a retry-count mechanism to send the data to S3 for manual inspection. 
